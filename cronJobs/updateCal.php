...
echo("Update calender is starting <br><br>");
$myfile = fopen("calender.js", "w") or die("Unable to open file!");
//$txt = file_get_contents('https://cors-anywhere.herokuapp.com/https://calendar.google.com/calendar/ical/ycg.org.uk_qneu0krg83dhmovjrnko6qmp6c%40group.calendar.google.com/public/basic.ics');
$curl_handle = curl_init();
curl_setopt($curl_handle, CURLOPT_URL, 'https://calendar.google.com/calendar/ical/ycg.org.uk_qneu0krg83dhmovjrnko6qmp6c%40group.calendar.google.com/public/basic.ics');
curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Your application name');
$txt = curl_exec($curl_handle);
curl_close($curl_handle);
echo("iCal file pulled from web:<br>");
echo($txt);
echo("<br>");
echo("<br>");
$iCal = new iCal($txt);
$currentDateTime = date('Y-m-d');
$yearPlusDateTime = date(strtotime("+2 months"));
$events = $iCal->eventsByDateBetween($currentDateTime, $yearPlusDateTime);
fwrite($myfile, "ALL_EVENTS = ");
foreach ($events as $date => $events) {
    foreach ($events as $event) {
        /*if ($event->recurrence == null){
            $timeS = date("H:i:s", strtotime("$event->dateStart -1 hour"));
            $timeE = date("H:i:s", strtotime("$event->dateEnd -1 hour"));
        }else {*/
        $timeS = date("H:i:s", strtotime($event->dateStart));
        $timeE = date("H:i:s", strtotime($event->dateEnd));
        //}
        $firstDate = date("Y:m:d", strtotime($event->dateStart));
        $secondDate = date("Y:m:d", strtotime($event->dateEnd));
        ...
        if (strpos(strtoupper($event->description), '!SOCIAL') !== false) {
            $social = true;
        } else {
            $social = false;
        }
        if (strpos(strtoupper($event->description), '!PRACTICE') !== false) {
            $practice = true;
        } else {
            $practice = false;
        }
        $output[] = [
            'startDate' => $date . " " . $timeS,
            'endDate' => $endDate,
            'title' => $event->summary,
            'location' => str_replace("\\", "", $event->location),
            'description' => str_ireplace("!SOCIAL", "", str_ireplace("!PRACTICE", "", str_ireplace("\\n", "", $event->description))),
            'social' => $social,
            'practice' => $practice
        ];
        echo("Event found: ");
