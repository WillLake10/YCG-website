<!DOCTYPE html>
<!--
Website: York Colleges Guild
Type: HTML5
Page: Ringing
Created: 20th April 2020
Last modified: 20th April 2020
Author: Will Lake
-->
<html lang="en">
<head>
    <script src="scripts/head.js" type="text/javascript"></script>
    <script src="scripts/divCalls.js" type="text/javascript"></script>
</head>
<body>

<div id="pageTop"></div>

<div id="splitterFull"></div>

<nav class="navbar navbar-expand-lg bg-light navbar-light sticky-top">
    <div id="navbarOuter"></div>
    <div class="collapse navbar-collapse" id="collapsibleNavbar">
        <div id="navbarItems"></div>
    </div>
</nav>

<script src="scripts/backToTopMove.js" type="text/javascript"></script>

<section id="pagetitle">
    <div class="container pt-3">
        <div class="row">
            <div class="col-md-3"></div>
            <div class="col-12 col-md-6">
                <div class="pagetitle">
                    Tower bell practices and other ringing
                </div>
            </div>
        </div>
    </div>
</section>

<section id="ringing">
    <div class="container pt-3 pb-3">
        <div class="row">
            <div class="col-md-6">
                <img src="images/ringing/ycg-ringers.jpg" class="img-fluid mx-auto d-block">
            </div>
            <div class="col-md-6">
                <p>We hold weekly practices alternating between The Spurriergate Center or St Lawrence's Church
                    in York. To see where we are this week refer to the events page</p>
                <p>At YCG, we ring a range of methods and activities - no matter your ability, there'll be
                    something for you here. Training is provided for complete beginners, while many of our older
                    members attend other practices throughout the city - they'd be more than happy to take you
                    along as a friendly face!.</p>
            </div>
        </div>
    </div>
</section>

<script src="scripts/splitterFull.js" type="text/javascript"></script><br>

<section id="spurriergate">
    <div class="container pt-3 pb-3">
        <div class="row">
            <div class="col-md-6">
                <div class="head_title text-ycgGreen">
                    Spurriergate
                </div>
                <p><span class="text-ycgGreen bold">Time:</span> 15:00-16:30</p>
                <p>For ringing at Spurriergate, we meet on campus at 14:30 or so and walk into the city together or we
                    sometimes head to 'spoons for lunch before practice, and possibly another pub after 'spoons'.
                    After practice a group of us normally go to Evensong ringing at St Lawrences at 17:15 and have
                    enough time to have a quick pint in the Waggon before ringing and sometimes end up in another pub
                    after</p>
                <p>See doves entry <a href="https://dove.cccbr.org.uk/detail.php?DoveID=YORK+++YSP">here</a></p>
            </div>
            <div class="col-md-6">
                <img src="images/ringing/spurriergate.jpg" class="img-fluid mx-auto d-block" width="100%">
            </div>
        </div>

        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        <div id="map-container-google-9" class="z-depth-1-half map-container-5" style="height: 300px">
                            <iframe src="https://maps.google.com/maps?q=Spurriergate&t=&z=15&ie=UTF8&iwloc=&output=embed"
                                    frameborder="0"
                                    style="border:0" allowfullscreen></iframe>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="head_title">
                    About Spurriergate
                </div>
                <p><span class="text-ycgGreen bold">Number of bells:</span> 6</p>
                <p><span class="text-ycgGreen bold">Tenor:</span> 8-2-14 in A</p>
                <p><span class="text-ycgGreen bold">Oldest Bell:</span> 554 Years (6th cast in 1466)</p>
                <p><span class="text-ycgGreen bold">Canons:</span> Yes</p>
                <p><span class="text-ycgGreen bold">Cast by:</span> 1 - George Dalton (1765), 2-5 - Samuel I Smith (1681), 6 - Thomas Innocent
                    (1466)</p>
                <p><span class="text-ycgGreen bold">Interesting Fact:</span> Some of the bells used to be part of York Minster's front 5</p>
            </div>
        </div>
    </div>
</section>

<script src="scripts/splitterFull.js" type="text/javascript"></script><br>

<section id="stlawrence">
    <div class="container pt-3 pb-3">
        <div class="row">
            <div class="col-md-6">
                <div class="head_title text-ycgGreen">
                    St Lawrences
                </div>
                <p><span class="text-ycgGreen bold">Time:</span> 15:30-17:00</p>
                <p>When we go to St Lawrences a group of us sometimes meet for food at Courtyard on campus before
                    walking up to St Lawrences. We often stay after our practice for the Evensong ringing at St
                    Lawrences at 17:15.</p>
                <p>See doves entry <a href="https://dove.cccbr.org.uk/detail.php?DoveID=YORK+++SLA">here</a></p>
            </div>
            <div class="col-md-6">
                <img src="images/ringing/stLawrences.jpg" class="img-fluid mx-auto d-block" width="100%">
            </div>
        </div>

        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        <div id="map-container-google-9" class="z-depth-1-half map-container-5" style="height: 300px">
                            <iframe src="https://maps.google.com/maps?q=St%20Lawrences%20church%2C%20york&t=&z=15&ie=UTF8&iwloc=&output=embed"
                                    frameborder="0"
                                    style="border:0" allowfullscreen></iframe>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="head_title">
                    About St Lawrences
                </div>
                <p><span class="text-ycgGreen bold">Number of bells:</span> 8</p>
                <p><span class="text-ycgGreen bold">Tenor:</span> 7-3-24 in A</p>
                <p><span class="text-ycgGreen bold">Oldest Bell:</span> 281 Years (Unused bell hung in ringing room cast in 1739)</p>
                <p><span class="text-ycgGreen bold">Canons:</span> No</p>
                <p><span class="text-ycgGreen bold">Cast by:</span> 1-2 & 5 - John Taylor Bellfounders Ltd (1999), 3-4 & 7-8 - John Taylor & Co
                    (1947), 6 - John Taylor & Co (Bellfounders) Ltd (1988)</p>
                <p><span class="text-ycgGreen bold">Interesting Fact:</span> The bells where put in in 1999 for the millennium</p>
            </div>
        </div>
    </div>
</section>

<section id="splitter" class="about bg-ycgGreen roomy-5">
    <div class="container">
        <div class="row">
            <div class="pageDivide">
                <br>
            </div>
        </div>
    </div>
</section>

<section id="otherringing">
    <div class="container pt-3 pb-3">
        <div class="head_title">
            Local Ringing around York members attend
        </div>
        <table class="table table-bordered">
            <thead>
            <tr>
                <th>Tower</th>
                <th>Time</th>
                <th>Day</th>
                <th>Ringing type</th>
                <th>Notes</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>St Lawrence</td>
                <td>9:30 - 10:15</td>
                <td>Sunday</td>
                <td>Service Ringing</td>
                <td>Followed by breakfast at Morisons with the ringers</td>
            </tr>
            <tr>
                <td>St Lawrence</td>
                <td>17:15 - 18:00</td>
                <td>Sunday</td>
                <td>Service Ringing</td>
                <td>After YCG practice</td>
            </tr>
            <tr>
                <td>St Lawrence</td>
                <td>7:30 - 21:00</td>
                <td>Monday</td>
                <td>Practice Night</td>
                <td>Followed by pub trip</td>
            </tr>
            <tr>
                <td>St Martins</td>
                <td>7:30 - 21:00</td>
                <td>Tuesday</td>
                <td>Practice Night</td>
                <td>Followed by pub trip</td>
            </tr>
            <tr>
                <td>St Wilfrid's</td>
                <td>7:30 - 21:00</td>
                <td>Thursday</td>
                <td>Practice Night</td>
                <td>Followed by pub trip</td>
            </tr>
            </tbody>
        </table>
    </div>
</section>

</body>
</html>
